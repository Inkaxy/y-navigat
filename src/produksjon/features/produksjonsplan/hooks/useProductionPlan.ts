...
export function useProductionPlan({ legalEntityId, date, criteria }: Args) {
  return useQuery({
    ...
    queryFn: async (): Promise<{
      ...
      // slik at diffen sammenligner samme rader. Selve summeringen ligger i
      // aggregateProductionLines (ren funksjon, dekket av tester).
      const rows = aggregateProductionLines(includedLines, criteria, {
        mainCategory: (id) => mainCatMap.get(id) ?? null,
        productionGroup: (id) => prodGroupMap.get(id) ?? null,
        customer: (id) => customerMap.get(id) ?? null,
        tourName: (t) => tourNameMap.get(t) ?? null,
      });
