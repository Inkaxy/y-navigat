...
/** ⌘K-kommandopalett — speilet fra NBOS Admin Hub. */
export function CommandPalette({ open, onOpenChange }: Props) {
  ...
  const handleNav = (path: string) => { navigate(path); close(); };
  const handleAppSwitch = (appUrl: string, appPath: string) => {
    window.location.href = `${appUrl}${appPath}?from=ordre`;
  };
  const handleTheme = (m: ThemeMode) => { setMode(m); close(); };
        ...
        <Command label="Kommandopalett" className="flex flex-col">
          ...
          <Command.List className="max-h-[420px] overflow-y-auto p-1">
            ...
            >
              ...
              {apps?.filter((a) => a.slug !== "ordre").map((app) => (
                <PaletteItem
                  key={app.id}
