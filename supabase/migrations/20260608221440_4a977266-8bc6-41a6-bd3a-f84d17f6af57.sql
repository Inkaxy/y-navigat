...
-- =========================================================================
-- D2: Migrate dining_mode 'dine_in' -> 'eatin'
-- =========================================================================
ALTER TABLE public.pos_transactions DROP CONSTRAINT IF EXISTS pos_transactions_dining_mode_check;
ALTER TABLE public.pos_transaction_lines DROP CONSTRAINT IF EXISTS pos_transaction_lines_dining_mode_override_check;

UPDATE public.pos_transactions SET dining_mode = 'eatin' WHERE dining_mode = 'dine_in';
UPDATE public.pos_transaction_lines SET dining_mode_override = 'eatin' WHERE dining_mode_override = 'dine_in';

ALTER TABLE public.pos_transactions
  ADD CONSTRAINT pos_transactions_dining_mode_check
  CHECK (dining_mode = ANY (ARRAY['takeaway'::text, 'eatin'::text]));

ALTER TABLE public.pos_transaction_lines
  ADD CONSTRAINT pos_transaction_lines_dining_mode_override_check
  CHECK (dining_mode_override IS NULL OR dining_mode_override = ANY (ARRAY['takeaway'::text, 'eatin'::text]));

-- =========================================================================
-- pos_record_sale — real implementation
-- =========================================================================
...
DECLARE
  ...
  v_ref_terminal uuid;
BEGIN
  -- Auth check (TODO: Kiosk-auth-modell vedtas i K.0)
  ...
  SELECT terminal_id, operator_id, legal_entity_id, status
    INTO v_terminal_id, v_operator_id, v_legal_entity_id, v_session_status
  ...
  -- 3) Validate reference transaction
  ...
  -- 4) Compute totals from lines (validate per-line shape)
  ...
  -- 5) Build canonical mva_breakdown from lines
  SELECT jsonb_agg(jsonb_build_object(
    'rate', rate,
    'base_excl_mva', base_excl,
    'mva_amount', mva_amt,
    'total_incl_mva', base_excl + mva_amt
  ) ORDER BY rate)
  INTO v_mva_agg
  FROM (
    SELECT
  ...
  --   < 0  = øre-bortavrundet (kvitteringen rundes ned til hele kroner el.l.)
  -- Tolerance ±0.01 for flytende-punkt-jitter.
  ...
  INSERT INTO public.pos_transactions (
    session_id, terminal_id, operator_id, legal_entity_id,
    ...
    p_session_id, v_terminal_id, v_operator_id, v_legal_entity_id,
  ...
  -- 10) Journal event (hash chain trigger fills event_hash/prev_hash)
