CREATE OR REPLACE FUNCTION public.pos_generate_z_report(p_terminal_id uuid, p_period_start timestamp with time zone, p_period_end timestamp with time zone)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
...
ALTER TABLE public.pos_z_reports
  ADD CONSTRAINT pos_z_reports_unique_period
  UNIQUE (terminal_id, period_start, period_end);