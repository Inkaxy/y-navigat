...
BEGIN
  ...
  v_label_payload := jsonb_build_object(
    'product_id',        v_base_product.id,
    'display_number',    v_base_product.display_number,
    'display_name',      v_base_product.display_name,
    'label_mode',        v_base_product.label_mode,
    'label_print_model', v_base_product.label_print_model,
  ...
  v_notes := v_base_product.display_name || ' (#' || v_base_product.display_number || ')';
  ...
  v_order_line := jsonb_build_object(
    ...
    'vat_rate',            COALESCE(v_base_product.mva_rate, 15),
    'notes',               v_notes
  );
