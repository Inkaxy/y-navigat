...
begin
  ...
  -- Uavklarte årsaker fra dokumentet skal overleve bekreftelsen av vare.
  ...
  -- Regn ut mengde i grunnenhet og pris per grunnenhet fra validerte kilde-
  -- verdier, med samme strenge enhetsregler som startpriskontrollen.
  ...
  if not coalesce(p_offer_start_price, true) then
    ...
    -- Uavklart uttrekk/valuta/grunnenhet: kilden er ikke gyldig for startpris.
  ...
  -- Linjene er ikke ferdig kontrollert før etterberegningen har kjørt, og
  -- bevarte dokumentårsaker settes tilbake.
  for r in select unnest(v_line_ids) as id loop
    v_arr := coalesce(
      (select array_agg(v) from jsonb_array_elements_text(v_preserved -> r.id::text) v),
      '{}'::text[]);
    v_arr := v_arr || 'recalculation_pending';
