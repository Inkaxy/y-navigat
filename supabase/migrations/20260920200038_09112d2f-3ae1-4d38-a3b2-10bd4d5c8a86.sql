...
AS $function$
  -- Nøyaktig tidspunkt for SISTE endring av grunnenheten. Historikken lagrer
  -- ingen enhet, så observasjoner registrert før dette tidspunktet kan ikke
  -- presenteres i dagens grunnenhet — heller ikke samme dag.
...
REVOKE ALL ON FUNCTION public.rm_unit_change_at(uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.rm_unit_change_at(uuid) TO authenticated, service_role;
...
begin
  ...
  -- Kjøp fra før grunnenheten ble endret er ikke sammenlignbare. Vi krever
  -- BÅDE at fakturadatoen er fra etter endringsdatoen (økonomisk rekkefølge)
  -- OG at raden faktisk ble registrert etter selve endringstidspunktet.
...
begin
  ...
  return jsonb_build_object(
    ...
    'unit_changed_at_ts', v_cutoff_ts,
    'on_date', p_on_date);
end;
$function$;