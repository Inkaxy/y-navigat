-- 1) Tillat operator_login i journalen
CREATE OR REPLACE FUNCTION public.pos_journal_append(p_terminal_id uuid, p_event_type text, p_operator_id uuid DEFAULT NULL::uuid, p_session_id uuid DEFAULT NULL::uuid, p_transaction_id uuid DEFAULT NULL::uuid, p_payload jsonb DEFAULT '{}'::jsonb)
 RETURNS bigint
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  IF p_event_type NOT IN (
    'operator_login','operator_logout','drawer_open','drawer_close','proforma_view',
...
-- 2) pos_record_sale med eksplisitt operatør
DROP FUNCTION IF EXISTS public.pos_record_sale(uuid, jsonb, jsonb, text, text, uuid, uuid, boolean);
...
CREATE OR REPLACE FUNCTION public.pos_record_sale(
  ...
  p_dining_mode text DEFAULT 'takeaway'::text,
  p_customer_id uuid DEFAULT NULL::uuid,
  p_reference_transaction_id uuid DEFAULT NULL::uuid,
...
BEGIN
  ...
  -- Kassasystemforskrifta: salget skal journalføres på operatøren som faktisk
  -- står i kassa, ikke den som åpnet sesjonen. Klienten sender operatør-id.
  IF p_operator_id IS NOT NULL AND p_operator_id <> v_operator_id THEN
    SELECT legal_entity_id INTO v_le FROM public.pos_terminals WHERE id = v_terminal_id;
    SELECT EXISTS (
      SELECT 1 FROM public.pos_operators o
      WHERE o.id = p_operator_id
        AND o.legal_entity_id = v_le
        AND COALESCE(o.is_active, true)
        AND (
          NOT EXISTS (SELECT 1 FROM public.pos_operator_terminals t WHERE t.operator_id = o.id)
          OR EXISTS (
            SELECT 1 FROM public.pos_operator_terminals t
            WHERE t.operator_id = o.id AND t.terminal_id = v_terminal_id
          )
        )
    ) INTO v_op_ok;
    IF NOT v_op_ok THEN
      RAISE EXCEPTION 'Operator % is not valid for this terminal', p_operator_id USING ERRCODE = '42501';
    END IF;
  ...
  SELECT receipt_number, receipt_sequence
    INTO v_receipt_number, v_receipt_sequence
  FROM public.pos_next_receipt_number(v_terminal_id);
...
REVOKE ALL ON FUNCTION public.pos_record_sale(uuid, jsonb, jsonb, text, text, uuid, uuid, boolean, uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.pos_record_sale(uuid, jsonb, jsonb, text, text, uuid, uuid, boolean, uuid) TO authenticated, service_role;