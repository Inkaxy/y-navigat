...
BEGIN
  ...
  -- Idempotens: hvis samme external_id fra portal allerede finnes — returner eksisterende
  IF v_external_id IS NOT NULL THEN
    SELECT id INTO v_existing_id FROM orders
     WHERE source = 'portal' AND source_external_id = v_external_id
     LIMIT 1;
  ...
  -- Pre-pass (validering + samle product_ids)
  ...
  LOOP
    ...
    IF v_cake.is_cake THEN
      ...
      IF COALESCE(v_product.lead_time_days, 0) > v_max_lead THEN
        v_max_lead := v_product.lead_time_days;
      END IF;
  ...
  -- Leveringsregel-validering: bestemmer om bestilling kan auto-bekreftes
  v_validation := public.validate_order_delivery_rules(
    v_customer.legal_entity_id,
    v_customer.id,
    v_delivery_date,
    v_delivery_tour_id,
    v_product_ids,
  ...
  RETURN jsonb_build_object(
    ...
    'final_customer_name', v_final_name,
    'total_incl_vat', v_total_incl,
    'status', v_initial_status,
    'auto_confirmed', v_passes,
