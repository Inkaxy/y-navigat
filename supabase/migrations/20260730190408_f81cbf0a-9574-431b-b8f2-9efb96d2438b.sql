-- 1) Åpen leveringspause (uten sluttdato) skal blokkere fra og med startdato
...
BEGIN
  ...
  SELECT
    ...
    (p_delivery_date IS NOT NULL
     ...
     -- pause_to = NULL betyr åpen pause uten sluttdato
     AND (dp.pause_to IS NULL OR p_delivery_date <= dp.pause_to)
    ...
    format('Leveringspause %s – %s%s',
      dp.pause_from,
      COALESCE(dp.pause_to::text, 'inntil videre'),
...
-- 2) Idempotens på ticket_replies
ALTER TABLE public.ticket_replies ADD COLUMN IF NOT EXISTS idempotency_key uuid;
CREATE UNIQUE INDEX IF NOT EXISTS ticket_replies_idempotency_key_uidx
  ON public.ticket_replies (idempotency_key)
