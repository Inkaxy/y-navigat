...
ALTER TABLE public.delivery_rules
  ADD COLUMN IF NOT EXISTS enforce_weekdays boolean NOT NULL DEFAULT false;

COMMENT ON COLUMN public.delivery_rules.enforce_weekdays IS
  'Når true: for rule_type i (order_deadline, available_tours, available_products) blokkeres ALSO leveringsdager som ikke matcher weekdays. Uten dette er weekdays kun et scope-filter (regelen gjelder KUN når leveransen faller på en av dagene).';
...
BEGIN
  ...
  -- der regelen har weekdays satt og enforce_weekdays = true, blokker også
  -- leveringsdager som ikke matcher — som en implisitt delivery_weekdays-regel.
  SELECT
    r.id AS rule_id,
    r.name AS rule_name,
    r.rule_type,
    ...
    AND (
      ...
      OR (
        p_product_ids IS NOT NULL AND (
          ...
          OR (r.allowed_product_group_ids IS NOT NULL AND p_product_group_ids IS NOT NULL
              AND r.allowed_product_group_ids && p_product_group_ids)
        )
      )
    )
