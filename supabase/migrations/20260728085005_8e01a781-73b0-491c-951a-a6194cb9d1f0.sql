CREATE OR REPLACE FUNCTION public.get_invoice_run_preview_lines(p_legal_entity_id uuid, p_run_date date, p_groups text[] DEFAULT NULL::text[])
 RETURNS TABLE(order_id uuid, order_number text, customer_number text, customer_name text, delivery_date date, tour_number integer, invoicing_group text, sum_excl_vat numeric, sum_incl_vat numeric, is_return boolean)
 LANGUAGE plpgsql
...
BEGIN
  ...
  ORDER BY COALESCE(c.display_name, '—'), o.delivery_date, o.order_number;
END;
$function$;

REVOKE EXECUTE ON FUNCTION public.get_invoice_run_preview_lines(uuid, date, text[]) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.get_invoice_run_preview_lines(uuid, date, text[]) TO authenticated;