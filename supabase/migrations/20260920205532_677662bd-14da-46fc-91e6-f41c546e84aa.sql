...
declare
  ...
  v_settings record;
  v_start jsonb := jsonb_build_object('attempted', false, 'created', false, 'reason', null);
...
begin
  ...
  select r.* into rm from public.raw_materials r where r.id = p_raw_material_id for share;
  if not found then
  ...
  -- Søsterlinjer må faktisk stå på samme faktura. Klientens liste er et ønske,
  -- ikke en sannhet.
  ...
  -- Startpris: bare når selskapet har slått det på, og aldri når det finnes en
  -- gyldig avtalepris på fakturadatoen.
